...
- default constructor 필수
  - CGLIB 프록시가 대상 클래스의 기본 생성자를 호출하므로.
- call constructor twice 
  - 자식 클래스 생성자 호출시, 부모 클래스의 생성자도 호출하게 된다(JAVA규약상)
  - 따라서, 실제 target객체 생성시, 프록시 객체 생성시에 부모클래스의 생성자가 호출되게 된다.
- final class, final method 사용 불가 (상속불가능해지므로)

[ 스프링의 해결책 ] 

위에 설명했던 각 프록시의 단점들을, 스프링에서는 어떻게 해결해왔는가?

- Spring3.2+, `CGLIB`를 스프링 내부에 패키징. 
  - -> 별도 라이브러리 추가 없이 CGLIB사용 가능
- Spring4+, `objenesis`라이브러리 이용한 CGLIB 단점 해결
  - -> 생성자 호출 없이 객체를 생성가능하게 했기에, 기본 생성자가 필요한 문제나, 생성자 2번 호출 문제 해결.
